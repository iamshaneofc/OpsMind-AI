...
/**
 * Ultimate Comprehensive Bot Testing
 * Tests all scenarios with real data, all roles, and verifies response quality
 */
...
const testResults = {
  ...
  byRole: {},
  byCategory: {},
  responseQuality: { realistic: 0, accurate: 0, needsImprovement: 0, inaccurate: 0 },
  edgeCases: { tested: 0, passed: 0 },
};
...
async function loadRealData() {
  ...
  const { data: orders } = await admin
    .from("orders")
    .select("id, order_id, order_number, company_id, warehouse_id, order_status, original_eta, revised_eta, delivery_date, delay_reason")
    .limit(50);
  realOrders = orders || [];

  const { data: invoices } = await admin
    .from("invoices")
    .select("id, invoice_id, invoice_number, company_id, customer_full_name, invoice_date, invoice_total_amount")
  ...
  console.log(`✓ Found ${realDelayedOrders.length} delayed orders, ${realOrderItems.length} order items`);
}

function createTestProfiles() {
  // Use real company and warehouse IDs
  const companyId = realCompanies[0]?.id || realCompanies[0]?.company_id || null;
  const warehouseId = realWarehouses[0]?.warehouse_id || null;
  
  // Find an order that belongs to a company
  const orderWithCompany = realOrders.find((o) => o.company_id);
  const testCompanyId = orderWithCompany?.company_id || companyId;
  
  // Find a warehouse that has orders
  const orderWithWarehouse = realOrders.find((o) => o.warehouse_id);
  const testWarehouseId = orderWithWarehouse?.warehouse_id || warehouseId;
...
async function executeToolFunction(toolName, args, profile) {
  ...
  switch (toolName) {
    case "getOrderStatus": {
      ...
      const originalETA = data.original_eta ? new Date(data.original_eta) : null;
      const calculatedStatus = data.order_status || 
        (data.delivery_date ? "Delivered" : 
         (originalETA && today > originalETA ? "Running Late" : "Work in Progress"));
    ...
    case "getDelayedOrders": {
      ...
      const query = supabase
        ...
        .or(`order_status.eq.Running Late,order_status.eq.RUNNING_LATE,original_eta.lt.${today}`)
...
async function buildTestScenarios() {
  const scenarios = [];

  // Get real data that exists and is linked
  ...
  // Find order that has invoices
  let orderWithInvoices = null;
  for (const order of realOrders) {
    const { data: invoiceOrders } = await admin
      ...
      .limit(1);
    if (invoiceOrders && invoiceOrders.length > 0) {
      orderWithInvoices = order;
      break;
    }
  }
  if (!orderWithInvoices) orderWithInvoices = testOrder;

  // Find invoice that exists
  let validInvoice = testInvoice;
  if (!testInvoice || !testInvoice.invoice_number) {
    validInvoice = realInvoices.find((i) => i.invoice_number) || testInvoice;
  }

  // Find order that warehouse user can access
  const warehouseOrder = realOrders.find((o) => o.warehouse_id && testWarehouse && o.warehouse_id === testWarehouse.warehouse_id) || testOrder;
  ...
  scenarios.push(
    ...
    },
    {
      ...
      tool: "getDelayedOrders",
      args: {},
      description: "Super Admin - Delayed orders",
      shouldWork: true,
    },
    {
      role: "super_admin",
      category: "Invoice Queries",
      query: validInvoice ? `Show me invoice ${validInvoice.invoice_number}` : "Show me invoice details",
      tool: "getInvoiceDetails",
      args: { invoiceNumber: validInvoice?.invoice_number || "" },
      description: "Super Admin - Invoice details",
      shouldWork: !!validInvoice,
    },
    {
      role: "super_admin",
      category: "Invoice Queries",
      query: `What invoices are linked to order ${orderWithInvoices.order_number}?`,
      tool: "getInvoicesByOrder",
      args: { orderNumber: orderWithInvoices.order_number },
  ...
  scenarios.push(
    ...
    },
    {
      ...
      tool: "getDelayedOrders",
      args: {},
      description: "Distributor - Delayed orders (own company)",
      shouldWork: true,
    },
    {
      role: "distributor",
    ...
    },
    {
      ...
      tool: "getWarehouseInventory",
      args: { warehouseId: testWarehouse?.warehouse_id || 1 },
      description: "Distributor - Warehouse inventory access denied",
      shouldWork: false,
      expectedError: "Access denied",
    }
  );
  ...
  if (testWarehouse) {
    scenarios.push(
      ...
      },
      {
        ...
        description: "Warehouse - Check inventory",
        shouldWork: true,
      },
      {
        role: "warehouse",
        category: "Order Queries",
        query: warehouseOrder ? `Where is Order ${warehouseOrder.order_number}?` : "Where is my order?",
        tool: "getOrderStatus",
        args: { orderNumber: warehouseOrder?.order_number || "" },
        description: "Warehouse - Order status (own warehouse)",
        shouldWork: !!warehouseOrder,
      },
      {
        role: "warehouse",
        category: "Order Queries",
        ...
        tool: "getDelayedOrders",
        args: {},
        description: "Warehouse - Delayed orders (own warehouse)",
        shouldWork: true,
      }
    );
  }
  ...
  scenarios.push(
    ...
    },
    {
      ...
      tool: "getOrderDrilldown",
      args: { orderNumber: testOrder.order_number },
      description: "Company Admin - Order drilldown",
      shouldWork: true,
    },
    {
      role: "company_admin",
    ...
    },
    {
      ...
      tool: "getInvoiceDetails",
      args: { invoiceNumber: validInvoice?.invoice_number || "" },
      description: "Company Admin - Invoice details",
      shouldWork: !!validInvoice,
    }
  );
  ...
  scenarios.push(
    {
      role: "super_admin",
      category: "Edge Cases",
      query: "Where is Order INVALID-ORDER?",
      tool: "getOrderStatus",
      args: { orderNumber: "INVALID-ORDER" },
      description: "Super Admin - Invalid order number",
      shouldWork: false,
    ...
    },
    {
      role: "distributor",
      category: "Edge Cases",
      query: "Show me invoice INVALID-INVOICE",
      tool: "getInvoiceDetails",
      args: { invoiceNumber: "INVALID-INVOICE" },
      description: "Distributor - Invalid invoice number",
      shouldWork: false,
      expectedError: "not found",
    },
    {
      role: "distributor",
      category: "Edge Cases",
      query: `Where is Order ${testOrder2?.order_number || "OTHER-ORDER"}?`,
      tool: "getOrderStatus",
      args: { orderNumber: testOrder2?.order_number || "OTHER-ORDER" },
      description: "Distributor - Order from other company (should be denied)",
      shouldWork: false,
      expectedError: "access denied",
    }
  );
  ...
  try {
    ...
    if (shouldWork) {
      ...
      if (result.error) {
        ...
        const expectedLower = expectedError ? expectedError.toLowerCase() : "";
        
        if (!expectedError || errorLower.includes(expectedLower)) {
          testResult = "PASS";
          ...
          testResult = "WARN";
          details.warning = `Expected error containing "${expectedError}" but got: ${result.error}`;
          testResults.edgeCases.tested++;
        }
...
async function runAllTests() {
  ...
  console.log("ULTIMATE COMPREHENSIVE BOT TESTING - ALL ROLES & SCENARIOS");
  console.log("=".repeat(80));
  ...
  const testScenarios = await buildTestScenarios();
  ...
  const reportPath = path.join(__dirname, "..", "..", "BOT_ULTIMATE_TEST_REPORT.json");
  fs.writeFileSync(reportPath, JSON.stringify(testResults, null, 2));
  console.log(`\n📄 Detailed report saved to: ${reportPath}`);

  // Calculate improved success rate
  const improvedRate = parseFloat(successRate);
  const previousRate = 85.2;
  const improvement = improvedRate - previousRate;
